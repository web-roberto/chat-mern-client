...
function IconSelect($$elementID, $$parameters) {
  ...
  this.refresh = function ($icons) {
    ...
    var horizontalIconNumber = Math.round(
  ...
  _View.showBox = function ($isShown) {
    ...
    if ($isShown) {
      ...
      _boxScroll = _boxScroll
        ? _boxScroll
        : new iScroll($$elementID + '-box-scroll');
    } else {
  ...
  _View.createUI = function ($parameters) {
    ...
    selectedBoxElement.style.height =
      $parameters.selectedIconHeight + $parameters.selectedBoxPadding * 2;
    ...
    _View.boxScrollElement.style.left =
      parseInt(selectedBoxElement.style.width) + 1;

    _View.boxScrollElement.style.width =
      ($parameters.iconsWidth + 2) * $parameters.vectoralIconNumber +
